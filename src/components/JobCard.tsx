...
const JobCard = ({ job, index = 0 }: JobCardProps) => {
  return (
    <Link to={`/jobs/${job.id}`} aria-label={`View ${job.title} at ${job.company}`}>
      <Card className="group cursor-pointer border transition-all hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5"
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <Badge className="text-xs bg-primary/10 text-primary border-primary/20 hover:bg-primary/20">{job.category}</Badge>
                <Badge variant="outline" className="text-xs border-accent/30 text-accent-foreground">{job.job_type}</Badge>
              </div>
